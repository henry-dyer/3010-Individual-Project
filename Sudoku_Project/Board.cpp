...
Board::Board(int setup[9][9])
{
    for (int i = 0; i < 9; i++)
    {
        for(int j = 0; j < 9; j++)
        {
            _board_[i][j] = new Square(setup[i][j], i, j);
...
void Board::give_hint()
{
    ...
    while(1)
    {
        ...
        {
           _board_[randRow][randCol]->set_value(_solution_[randRow][randCol]);
           break;
        }
    }
    return;
